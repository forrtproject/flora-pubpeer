"""Test copy of send_emails.py — sends the rows in data/email_test.csv instead
of data/formatted_emails.csv, so the real send/resume logic can be exercised
against your own test addresses before running the real campaign.

Each row of email_test.csv has the same shape as formatted_emails.csv:
  - email_addresses: one or more recipient addresses, separated by "; ".
    When a row has several, they are all addressed on the SAME email, not
    sent as separate emails.
  - html: the full, ready-to-send HTML body for that email (its "To:"
    preview banner still shows the real author address(es), even though
    email_addresses has been swapped to a test inbox).

Uses its own log file (data/test_send_log.csv) and does NOT touch
data/send_log.csv or count against the real campaign's daily quota.
"""
...
FORMATTED_EMAILS_PATH = os.path.join(PROJECT_ROOT, "data", "email_test.csv")
SEND_LOG_PATH = os.path.join(PROJECT_ROOT, "data", "test_send_log.csv")
